...
rb_red_blk_node * RBTreeInsert(rb_red_blk_tree* tree, void* key, void* info) {
  ...
  /*printf("RBTreeInsert(%p, %d, %p)\n", tree, *(int*)key, info);*/
  /*printf("Inserir Thread com TID=%d\n", *(int*)info);*/

  /* Actualizar first */
  if(tree->first == NULL) tree->first = x;
  if(*(int*)key < *(int*)(tree->first->key)) tree->first = x;
  /* Actualizar first */
...
struct _sthread * RBDelete(rb_red_blk_tree* tree){
  rb_red_blk_node* y;
  rb_red_blk_node* x;
  rb_red_blk_node* nil=tree->nil;
  rb_red_blk_node* root=tree->root;
  rb_red_blk_node* z = tree->first;
  rb_red_blk_node* aux;
  struct _sthread* s;

  /*printf("\nA apagar da arvore %p o elemento %p\n", tree, z);*/

  /*RBTreePrint(tree);*/

  if(tree->first == NULL) {
    /*printf("A arvore esta vazia - vamos devolver NULL");*/
    return NULL;
  }
  /*printf("A remover a thread localizada em %p\n", z);*/
  s = z->info;
...
int RBSearch(rb_red_blk_tree* tree, rb_red_blk_node *n, struct _sthread* t) {
	if (n == NULL) return 0;
	if (n == tree->nil) return 0;
	if (n->info == NULL) { printf("ERRO\n"); while(1); }
	if (n->info == t) return 1;
	return RBSearch(tree, n->left, t) || RBSearch(tree, n->right, t);
}
